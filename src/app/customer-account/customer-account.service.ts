...
export class CustomerAccountService {
  ...
  loadCurrentUser(token:string)
  {
     ...
     return this.http.get<Login>('https://localhost:7239/CustomerAccount/user',{headers}).pipe(
      map(user=>{
  ...
  login(values:any)
  {
    return this.http.post<Login>('https://localhost:7239/CustomerAccount/login',values).pipe(
      map(customer=>{
